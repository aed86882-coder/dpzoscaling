...
# Build command with optional batch_size and max_length
CMD_ARGS=""
if [ ! -z "$BATCH_SIZE" ]; then
    CMD_ARGS="$CMD_ARGS --per_device_train_batch_size $BATCH_SIZE --per_device_eval_batch_size $BATCH_SIZE"
fi
if [ ! -z "$MAX_LENGTH" ]; then
    CMD_ARGS="$CMD_ARGS --max_length $MAX_LENGTH"
fi
